...
Config::Config() : ip("127.0.0.1"), port(4444), limit_client_body_size(1000000) {
...
Config::Config(const std::string &ip, int port) : ip(ip), port(port), limit_client_body_size(1000000) {
  if (inet_addr(ip.c_str()) == INADDR_NONE) {
    std::cerr << "inet_addr error" << std::endl; //TODO: решить, что делать при ошибке
  }
  index.push_back("index.html");
...
Config::Config(const Config &x) : ip(x.ip), port(x.port), limit_client_body_size(x.limit_client_body_size), index(x.index) {}
